...
export async function POST(request: NextRequest) {
  try {
    ...
    const formId = process.env.KIT_FORM_ID_RACCOURCIS;

    if (!apiKey || !formId) {
      console.error('KIT_API_KEY or KIT_FORM_ID_RACCOURCIS is not configured');
      return NextResponse.json(
    ...
    if (kitResponse.ok) {
      return NextResponse.json({
        success: true,
        message: 'Votre guide des raccourcis est en route ! Vérifiez votre boîte mail.',
      });
    }
    ...
    console.error('Guide raccourcis signup error:', error);
    return NextResponse.json(
      { success: false, message: 'Une erreur est survenue. Veuillez réessayer.' },
      { status: 500 }
    );
